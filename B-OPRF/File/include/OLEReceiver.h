...
class OLEReceiver
{
    ...
    vec_ZZ_p compute_ole(vec_ZZ_p x, int inputLength, vec_ZZ_p Alpha, vec_ZZ_p Beta)
    {
       ...
       {     for(int j = 0; j < n; j++)
        ...
        cout << "Receiver: Total communication for sending OLE vector: " << (sizeof(block)*n*num_encoding_instances)/1000 << "KB" << endl;
        ...
        */
        ...
        end_time = high_resolution_clock::now();
        duration = duration_cast<milliseconds>(end_time - start_time);
        std::cout << "Receiver OT Cost: " << ios[1]->counter << "bytes" << endl; 
        std::cout << "Receiver: OT time for " << ot_length << " inputs: " << duration.count() << "ms" << endl; 
