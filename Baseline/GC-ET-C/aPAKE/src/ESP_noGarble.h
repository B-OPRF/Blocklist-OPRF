...
vector<vector<int>> deterTypeOne(vector<int> s) {
  // input: a string that is represented in an array of ascii characters
  // output: all type one substrings
...
vector<vector<int>> deterTypeTwo(vector<int> s) {
  // input: a string that is represented in an array of ascii characters
  // output: all type two substrings
  vector<vector<int>> ret;
  int vecSize = s.size();
...
int findDif(vector<int> s1, vector<int> s2) {
  ...
  // output: the first digit where the two vectors are different
  int index = s1.size()-1;
  do {
...
// input: an ascii-encoded character
// output: its binary representation form (1 byte)
{
  vector<int> ret;
  while (n / 2 != 0) {
    ret.push_back(n % 2);
...
bool is_in(vector<int> pair, vector<vector<int>> vec) {
  // input: a value pair, a vector of ranges
  // output: whether the pair is within any range in the vector
...
vector<vector<int>> divideType(vector<int> str) {
  // input: a string that is represented in an array of ascii characters
  // divide the characters into substrings with their ESP substring type specified
...
vector<int> find_closest(int length, vector<int> landmark) {
  // input: string length and landmark vector
  // for each character, find its closest landmark
  // output: closest landmark of each string character
...
vector<vector<int>> divideBlock(vector<vector<int>> pos) {
  // input: a vector of substrings with their ESP substring type specified
  // divide each substring into blocks of 2 or 3 characters according to its ESP substring type
  // output: a vector of ascii blocks
...
vector<int> sha3(int *input, int length){
  // input: an integer array and array length
  // output: sha3 result of the integer array as a 256 bit vector
  vector<int> ret;
...
{
  // input: a vector of ascii blocks
  // make an ESP tree with sha3 for nodes
  // output: set of all ESP-tree nodes
...
vector<vector<int>> ESP_Type(vector<int> emp_array) {
  // input: a string that is represented in an array of ascii characters
  // make an ESP set with sha3 for nodes without block type division
  // output: set of all ESP-tree nodes
